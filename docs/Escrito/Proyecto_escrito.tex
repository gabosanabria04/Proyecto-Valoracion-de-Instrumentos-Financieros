...
En el enfoque Ho–Lee en tiempo discreto, se construyó un árbol binomial para la tasa corta con pasos mensuales $\Delta t=\tfrac{1}{12}$ años. En cada nodo la tasa puede subir o bajar mediante los factores
\[
u(t) \text{ y } d(t)
\]
y se define
\[
k=\frac{d(2)}{u(2)}\in(0,1).
\]
...
$$
Para cada mes, la tasa en un nodo se obtiene ajustando ese forward con el desplazamiento del árbol:
$$
r_{t+\Delta t} =
...
\begin{enumerate}
  ...
  \item el precio de la acción $S_t$ bajo un esquema binomial.
\end{enumerate}
